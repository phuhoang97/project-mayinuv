...
function DetailProduct() {
  const [mainImage, setMainImage] = useState(null); // State để lưu trữ đường dẫn của ảnh chính
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `https://ecommerce-printer-be.vercel.app/api/products/${id}`
        );
        if (!response.ok) {
        ...
        setData(jsonData);
        const imagesArray = JSON.parse(jsonData.data.images);
  ...
  const handleThumbnailClick = (imageSrc) => {
    setMainImage(imageSrc); // Cập nhật ảnh chính với ảnh được click
  };
              ...
              <Row gutter={[16, 16]}>
                {alldata.data.map((data, index) => (
                  <Col key={index} xs={24} sm={12} md={8} lg={6} xl={6}>
                    <Link to={`/products/${data.id}`} onClick={() => handleClick(data.id)}>
